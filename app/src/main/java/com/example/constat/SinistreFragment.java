...
import com.example.constat.Database.SinistreDatabase;
...
public class SinistreFragment extends Fragment {
  ...
  // Function to show the DescriptionSinistreFragment when a sinistre is clicked
  private void showDescriptionFragment(Sinistre sinistre) {
    DescriptionSinistreFragment descriptionSinistreFragment = new DescriptionSinistreFragment();
    Bundle bundle = new Bundle();
    bundle.putInt("sinistreId", sinistre.getId()); // Use "sinistreId" as the key to match the retrieval in DescriptionSinistreFragment
    ...
    descriptionSinistreFragment.setArguments(bundle);

    FragmentTransaction transaction = getParentFragmentManager().beginTransaction();
    transaction.replace(R.id.fragment_container, descriptionSinistreFragment);
    transaction.addToBackStack(null);
    transaction.commit();
  }
}
